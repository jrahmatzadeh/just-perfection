...
var Prefs = class
{
    ...
    constructor(dependecies, shellVersion)
    {
        ...
        this._window_width = 200;
        this._window_height = 750;
    ...
    getMainPrefs(UIFilePath, binFolderPath, gettextDomain)
    {
        ...
        this._applyShellVersionToObjects();
        
        // default window size
        let signal = obj.connect('realize', () => {
            let window = obj.parent.parent;
            window.default_width = this._window_width;
            window.set_size_request(this._window_width, this._window_height);
            window.resize(this._window_width, this._window_height);
        });
    ...
    /**
     * make the ui objects hidden if it is not related
     * to the current shell version 
     *
     * @return void
     */
    _applyShellVersionToObjects()
    {
        if (this._shellVersion < 40) {
            this._builder.get_object('window_picker_wrapper').visible = false;
        }
    }
