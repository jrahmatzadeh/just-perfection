...
var API = class
{
     ...
     *  no-app-menu-icon
     *  clock-menu-no-gap
     *  clock-menu-no-gap-start
     *  clock-menu-no-gap-end
     *
     * @return string
     */
    ...
    panelSetPosition(position)
    {
        ...
        if (position === 'top') {
            ...
            let topX = pMonitor.x;
            let topY = pMonitor.y;
            this._main.layoutManager.panelBox.set_position(topX, topY);
            this.UIstyleClassRemove(this._getAPIClassname('bottom-panel'));
            return;
        }
        
        let BottomX = pMonitor.x;
        let BottomY = pMonitor.height - this._originals['panelHeight'];
        
        this._main.layoutManager.panelBox.set_position(BottomX, BottomY);
     ...
     *
     * @param int pos see self::PANEL_BOX_POSITION
    ...
    clockMenuPositionSet(pos, offset)
    {
        ...
        this._clockMenuNoGapSet(pos, toIndex, childLength);
    }
    
    /**
     * set no gap for clock menu
     *
     * @param int pos see self::PANEL_BOX_POSITION
     * @param int index current clock menu index in the position
     * @param int maxIndex maximum child index in the position
     *
     * @return void
     */
    _clockMenuNoGapSet(pos, index, maxIndex)
    {
        let dateMenu = this._main.panel.statusArea['dateMenu'];
        let classname;
        ...
        if (pos === this.PANEL_BOX_POSITION.CENTER) {
            return;
        }
        
        if (pos === this.PANEL_BOX_POSITION.RIGHT && index >= maxIndex) {
            classname = this._getAPIClassname('clock-menu-no-gap-end');
        } else if (pos === this.PANEL_BOX_POSITION.LEFT && index === 0) {
            classname = this._getAPIClassname('clock-menu-no-gap-start');
        } else {
            classname = this._getAPIClassname('clock-menu-no-gap');
        }
        
        dateMenu.add_style_class_name(classname);
    }
    
    /**
     * revert no gap for clock menu
     *
     * @return void
     */
    _clockMenuNoGapDefault()
    {
        let dateMenu = this._main.panel.statusArea['dateMenu'];
        
        let classname = this._getAPIClassname('clock-menu-no-gap');
        let classnameStart = this._getAPIClassname('clock-menu-no-gap-start');
        let classnameEnd = this._getAPIClassname('clock-menu-no-gap-end');
        
        dateMenu.remove_style_class_name(classname);
        dateMenu.remove_style_class_name(classnameStart);
        dateMenu.remove_style_class_name(classnameEnd);
    }
}
