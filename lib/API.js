    ...
    _startSearchSignal(add)
    {
        ...
        if (!add) {
            if (this._searh_page_changed_signal) {
                this._main.overview.viewSelector.disconnect(
        ...
        this._searh_page_changed_signal
        = this._main.overview.viewSelector.connect('page-changed', () => {
        
            if (this._searchEntryVisible) {
                return;
            }
        
            let isInSearch = this._main.overview.viewSelector.getActivePage() === 3;
