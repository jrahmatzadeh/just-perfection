...
var Manager = class
{
    ...
    registerSettingsSignals()
    {
        ...
        this._settings.connect('changed::no-panel-corner', () => {
            this._applyNoPanelCorner(false);
        });
    ...
    /**
     * apply no panel corner settings
     *
     * @param bool forceOriginal
     *
     * @return void
     */
    _applyNoPanelCorner(forceOriginal)
    {
        if (forceOriginal || !this._settings.get_boolean('no-panel-corner')) {
            this._api.panelCornerDefault();
        } else {
            this._api.panelCornerDisable();
        }
    }
